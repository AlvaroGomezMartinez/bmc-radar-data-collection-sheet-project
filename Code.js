/**
 * Radar Data Collection Sheet Project
 * 
 * Objective: Create a data collection sheet that will be used for placement decision making by
 * stakeholders.
 * 
...
function setupWeeklyBehaviorSheets() {
  ...
  // Ask how many weeks
  const weekResponse = ui.prompt("Number of Weeks", "How many weeks would you like to create?", ui.ButtonSet.OK_CANCEL);
  ...
  for (const sheetName of sheetNames) {
    ...
    const dateCells = ["C2", "E2", "G2", "I2", "K2"];
    for (let i = 0; i < 5; i++) {
      summarySheet.getRange(2, colIndex).setFormula(`='${sheetName}'!${dateCells[i]}`);
      colIndex++;
    }
  }
  ...
  for (let behaviorRow = 9; behaviorRow <= 15; behaviorRow++) {
    ...
    for (const sheetName of sheetNames) {
      ...
      for (let i = 0; i < 5; i++) {
        ...
        summarySheet.getRange(behaviorRow - 6, col).setFormula(`='${sheetName}'!${cell}`);
        col++;
      }
    }
  }
  ...
  for (let i = 0; i < behaviorRowIndexes.length; i++) {
    ...
    for (const emoji of emojis) {
      ...
      for (const sheetName of sheetNames) {
        const refs = columnLetters.map(col => `'${sheetName}'!${col}${weekRow}`).join(" & ");
        const part = `(LEN(${refs}) - LEN(SUBSTITUTE(${refs}, "${emoji.symbol}", ""))) / 2`;
        formulaParts.push(part);
      }

      const fullFormula = `=${formulaParts.join(" + ")}`;
      summarySheet.getRange(`${emoji.col}${summaryRow}`).setFormula(fullFormula);
    }
  }
